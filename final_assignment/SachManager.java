...
public class SachManager implements IQuanLySach {
    ...
    public void ThongKe(){
        ...
        for (SACH s : List.values()){
            dem += 1;
            if(s instanceof Novel)
              demN += 1;
            if(s instanceof TEXTBOOK)
              demT += 1;
        }
        System.out.println("\n===== THONG KE SACH TRONG KHO =====");
        System.out.println("Tong so luong sach: " + dem);
        System.out.println("  ├─ So Sach tieu thuyet (Novel): " + demN);
        System.out.println("  └─ So Sach giao khoa (Textbook): " + demT);
    }
}
